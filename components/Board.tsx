...
const resond = localFont({
    src: '../public/fonts/Resond.otf',
    display: 'swap',
});
...
// Architectural definition of columns with turquoise gradient
...
export const Board: React.FC<Props> = ({ tasks, projects, activeProjectId }) => {
    ...
    // ==========================================
    // DRAG AND DROP PROTOCOL (Native APIs)
    // ==========================================
    ...
    // Filter tasks to NOT show those belonging to archived projects
    ...
    return (
        <div className="flex-1 overflow-x-auto p-6 custom-scrollbar relative z-10">
            ...
            <div className="mb-8 flex items-center justify-between border-l-4 border-[#00ffff] pl-4">
                <div>
                    <h1 className={`text-3xl font-black uppercase tracking-widest text-white flex items-center gap-3 ${pondar.className}`}>
                ...
                {/* TRIGGER BUTTON WITH 2PX WRAPPER (Perfect bright diagonal) */}
                <div className="shrink-0" style={{ perspective: '1000px' }}>
                    <div
                        className={`p-[2px] transition-all duration-300 shadow-[0_0_15px_rgba(0,255,255,0.2)] transform-gpu
                            ${activeProjectId ? 'hover:scale-105 active:scale-95' : 'opacity-50 grayscale'}`}
                        style={{ background: '#00ffff', clipPath: 'polygon(10% 0, 100% 0, 90% 100%, 0 100%)' }}
                    >
                        <button
                            onClick={() => setIsModalOpen(true)}
                            disabled={!activeProjectId}
                            className={`group relative bg-[#0b2229] px-6 py-2 flex items-center gap-3 font-black uppercase tracking-[0.2em] text-xs transition-colors duration-300
                                ${activeProjectId ? 'hover:bg-[#00ffff] text-[#00ffff] hover:text-black cursor-pointer' : 'text-gray-500 cursor-not-allowed'}`}
                            style={{ clipPath: 'polygon(10% 0, 100% 0, 90% 100%, 0 100%)' }}
                        >
                            <Plus size={18} className={`transition-transform duration-500 ${activeProjectId ? 'animate-pulse group-hover:rotate-180' : ''}`} />
                            <span>Add task</span>
                        </button>
                    </div>
                </div>
      ...
      ========================================== */}
            ...
            `}
                    >
                        {/* Column Header */}
                        <div
                            className={`flex items-center gap-2 mb-4 bg-black/40 p-3 text-lg tracking-[0.2em] shadow-md transition-colors duration-300
              ...
              `}
                        ...
                        <div className="flex-1 overflow-y-auto overflow-x-hidden px-3 pb-8 custom-scrollbar relative">
                            {/* Exclusive column background scanlines */}
